import { phaseOffset, signalOffset, efficientSignalOffset } from '../_fft';

describe('phaseOffset()', () => {
  it('produces the correct phase', () => {
    const RES_1 = [1, 0, -1, 0, 1, 0, -1];
    expect(phaseOffset(0, 7)).toEqual(RES_1);

    const RES_2 = [0, 1, 1, 0, 0, -1, -1];
    expect(phaseOffset(1, 7)).toEqual(RES_2);

    const RES_3 = [0, 0, 1, 1, 1, 0, 0, 0, -1];
    expect(phaseOffset(2, 9)).toEqual(RES_3);

    const RES_4 = [0, 0, 1, 1, 1, 0, 0, 0, -1, -1, -1, 0];
...
describe('efficientSignalOffset()', () => {
  it('offsets the input correctly', () => {
    const input = 12345678;
    const inputArr = input
      .toString()
    ...
    const result1 = efficientSignalOffset(inputArr);
    expect(+result1.join('')).toEqual(48226158);

    const result2 = efficientSignalOffset(result1);
    expect(+result2.join('')).toEqual(34040438);

    const result3 = efficientSignalOffset(result2);
    expect(result3.join('')).toEqual('03415518');
  });
});
